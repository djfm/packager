...
function compile (filePath, code, options) {
    if (options && options.extensions) {
        var extension = _.find(options.extensions, function (ext) {
            return ext.extension === path.extname(filePath);
        });

        if (extension && extension.compile) {
            return extension.compile(code);
        }
    }
    return code;
}

function doListRequiredFiles (mainPath, sourcesByAbsolutePath, options, history) {
    var d = q.defer();

    if (Object.prototype.hasOwnProperty.call(sourcesByAbsolutePath, mainPath)) {
        d.resolve([]);
    } else {
        fs.readFile(mainPath, function (err, data) {
            if (err) {
                ...
                var code = compile(mainPath, data.toString(), options);

                sourcesByAbsolutePath[mainPath] = code;

                var children = listRequiredFilesInSourceCode(code).map(function (rf) {
                    rf.requiredFrom = mainPath;
                    var newHistory = _.clone(history);
                    newHistory.requiredFiles.concat(rf);
                    return resolver.resolveRequiredFilePath(rf, options, newHistory).then(function (rf) {
                        return doListRequiredFiles(rf.path, sourcesByAbsolutePath, options, newHistory).then(function (files) {
...
function package (entryPoint, options) {
    ...
    return listRequiredFiles(entryPoint, options).then(function (data) {
        ...
        var map = require('./source-mapper').buildSourceMap(code).toString();
