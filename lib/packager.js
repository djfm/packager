...
function doListRequiredFiles (mainPath, alreadySeenPaths, options) {
    var d = q.defer();
        ...
        fs.readFile(mainPath, function (err, data) {
            if (err) {
                ...
                var children = listRequiredFilesInSourceCode(jsCode).map(function (rf) {
                    ...
                    return resolver.resolveRequiredFilePath(rf, options).then(function (rf) {
                        return doListRequiredFiles(rf.path, alreadySeenPaths, options).then(function (files) {
...
function listRequiredFiles (mainPath, options) {
    var sources = {};
    return doListRequiredFiles(mainPath, sources, options).then(function (files) {
        return {
            files: files,
            sources: sources
        };
