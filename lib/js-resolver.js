...
function resolveRequiredFilePath (rf, config, history) {
    ...
    function handleOverrides () {
        if (!history || !history.overrides) {
            return;
        }
        // console.log(rf, JSON.stringify(history));
        if (_.has(history.overrides, rf.raw)) {
            raw = history.overrides[rf.raw];
        }
    }
    ...
    if (_.has(builtins, raw)) {
        rf.path = builtins[raw];
        return q(rf);
    }
                                ...
                                if (err) {
                                    ...
                                    if (json.browser) {
                                        if (typeof json.browser !== "string") {
                                            history.overrides = json.browser;
                                        } else {
    ...
    return getCandidateLocations().then(addExtensions).then(function (candidates) {
        ...
        if (null === pathOrNull) {
            var chain = _.map(history.requiredFiles, function (h) {
