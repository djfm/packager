...
function resolveRequiredFilePath (rf, config) {
    ...
    }, q.reject(couldNotResolveError(rf)));
}
...
function doResolve (rf, path) {
    ...
    fs.exists(path, function (yes) {
        if (yes) {
            ...
            d.reject(couldNotResolveError(rf));
        }
    });
...
function couldNotResolveError (rf) {
    return new Error('Could not resolve required file "' + rf.raw + '" (required from: "' + rf.from + '").');
}
